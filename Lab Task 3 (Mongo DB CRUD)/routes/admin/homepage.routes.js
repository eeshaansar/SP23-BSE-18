...
// Route to display the main page with all categories
...
router.get('/category/:categoryId', async (req, res) => {
    try {
        ...
        // Render the page with the category and corresponding products
...
// Route for category details with product information
router.get('/details/:categoryId/:productId', async (req, res) => {
    try {
        const { categoryId, productId } = req.params;
        console.log("Category ID for details:", categoryId, "Product ID:", productId);

        // Validate if categoryId and productId are valid MongoDB ObjectIds
        if (!categoryId.match(/^[0-9a-fA-F]{24}$/) || !productId.match(/^[0-9a-fA-F]{24}$/)) {
        ...
        res.render('website/productDetail', { category, product, layout: 'layout' });
    } catch (err) {
